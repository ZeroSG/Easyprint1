...
- (void)printText:(NSString*)text encoding:(CFStringEncoding)encoding
{
    while (text.length > 20)
    {
        NSString* strPrint = [text substringToIndex:20];
        [self writeData:[strPrint dataUsingEncoding:CFStringConvertEncodingToNSStringEncoding(encoding)]];//kCFStringEncodingGB_18030_2000
    ...
    if (text.length > 0)
    {
        [self writeData:[text dataUsingEncoding:CFStringConvertEncodingToNSStringEncoding(encoding)]];
    }
